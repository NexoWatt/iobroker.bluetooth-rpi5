...
class BlueZ extends EventEmitter {
  ...
  async init() {
    ...
    // Ensure adapter is powered on (best effort)
    try {
      const all = await this.adapterProps.GetAll('org.bluez.Adapter1');
      const powered = Boolean(unwrapVariant(all.Powered));
      if (!powered) {
        this._log.warn('Bluetooth adapter is powered off - attempting to enable Powered=true');
        await this.adapterProps.Set('org.bluez.Adapter1', 'Powered', new Variant('b', true));
      }
    } catch (e) {
      // Not fatal (may require polkit)
      this._log.debug(`Adapter Powered check/set failed (ignored): ${e.message}`);
    }
   ...
   * @returns {Promise<Array<{path: string, address: string, name?: string, alias?: string, rssi?: number, connected?: boolean, paired?: boolean, trusted?: boolean}>>}
   */
  async listDevices() {
    ...
    for (const [path, ifaces] of entries(managed)) {
      ...
      // Resolve Device1 props
      let props = null;
      if (ifaces instanceof Map) {
      ...
      const alias = unwrapVariant(props.Alias);
      ...
      devices.push({
        ...
        trusted: typeof trusted === 'boolean' ? trusted : undefined,
      });
    }
    return devices;
  }
  ...
  /**
   * Returns detailed Device1 info for a given device object path.
   * @param {string} devicePath
   * @returns {Promise<{path: string, address?: string, name?: string, alias?: string, rssi?: number, connected: boolean, paired: boolean, trusted: boolean, servicesResolved: boolean}>}
   */
  async getDeviceInfoByPath(devicePath) {
    if (!this.bus) throw new Error('BlueZ not initialized');

    const obj = await this.bus.getProxyObject(BLUEZ_SERVICE, devicePath);
  ...
  /**
   * Returns device info by MAC address (null if unknown).
  ...
  async getDeviceInfo(address, opts = {}) {
    ...
    const path = await this.waitForDevicePath(address, { timeoutMs, scan });
    if (!path) return null;

    return this.getDeviceInfoByPath(path);
  }

  /**
   * Pairs a device (tries "Just Works").
   * Note: If the device requires a passkey/PIN, this might fail unless an Agent is available.
   * @param {string} address
   * @param {{timeoutMs?: number, scan?: boolean}} [opts]
   */
  async pairDevice(address, opts = {}) {
    ...
    try {
      ...
      // Some devices may already be paired
      if (!/AlreadyExists|already exists|AlreadyPaired|already paired/i.test(String(e.message || ''))) {
        throw e;
      }
    }

    // Wait until Paired=true
    const start = Date.now();
    while (Date.now() - start < timeoutMs) {
      try {
        const all = await props.GetAll('org.bluez.Device1');
        if (Boolean(unwrapVariant(all.Paired))) break;
      } catch {
        // ignore
      }
  ...
  /**
   * Sets Device1.Trusted.
   * @param {string} address
   * @param {boolean} trusted
   * @param {{timeoutMs?: number, scan?: boolean}} [opts]
   */
  async setTrusted(address, trusted = true, opts = {}) {
    if (!this.bus) throw new Error('BlueZ not initialized');
    ...
    await props.Set('org.bluez.Device1', 'Trusted', new Variant('b', Boolean(trusted)));

    return this.getDeviceInfoByPath(path);
  }

  /**
   * Removes a device from BlueZ (unpair/forget).
   * @param {string} address
   * @param {{timeoutMs?: number, scan?: boolean}} [opts]
   */
  async removeDevice(address, opts = {}) {
    ...
    try {
      ...
      if (/DoesNotExist|NotExist|NotFound|not found/i.test(String(e.message || ''))) {
        return false;
      }
      throw e;
    }
